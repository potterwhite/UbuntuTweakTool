...
func_1_1_setup_env(){
	...
	_DONOT_USE_BUILD_SCRIPT_PATH="sudo ${ENV_K_SDK_DIR}/build.sh"	
...
func_1_2_show_usage() {
    ...
    echo -e "    u, upgrade    Perform online kernel upgrade"
    echo -e "                  - Copies kernel image to temporary location"
    echo -e "                  - Burns image to board"
    echo -e "                  - Reboots system automatically\n"

    echo -e "EXAMPLES:"
    echo -e "    $(basename $0) build     # Full build process"
    echo -e "    $(basename $0) b         # Same as above"
    echo -e "    $(basename $0) upgrade   # Online upgrade"
    echo -e "    $(basename $0) u         # Same as above\n"

    echo -e "NOTES:"
    echo -e "    - Commands are case-insensitive"
    echo -e "    - Partial command matching is supported"
    echo -e "    - Build process requires sudo privileges"
    echo -e "    - Upgrade requires SSH access to board\n"
